...
Legacy no-screen Show creation remains temporarily compatible and does not create ShowSeats. Customer booking now uses ShowSeat availability for initialized screen-aware Shows while preserving `Booking.seats` and `Show.bookedSeats` string labels. Booking confirmation for initialized Shows synchronizes `Show.bookedSeats`, ShowSeats, and Booking in a MongoDB transaction, so production should remain on transaction-capable MongoDB such as Atlas or another replica-set-backed deployment.
